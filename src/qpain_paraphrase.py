# ABOUTME: Calibrated paraphrase generation for Q-Pain race bias experiment
# ABOUTME: Targets each vignette's race-swap token distance using GPT with async parallelism
...
REFUSAL_CORRECTION = "That was a refusal. Please provide only the paraphrased vignette, nothing else."


def format_initial_prompt(vignette, target_pct, orig_token_count, target_edits, tolerance=0.5):
    """Format the initial calibrated paraphrasing prompt for a vignette."""
...
def format_synonym_prompt(vignette, num_words):
    """Format a synonym replacement prompt for small target changes."""
    return SYNONYM_PROMPT_TEMPLATE.format(vignette=vignette, num_words=num_words)


def merge_paraphrases(df, paraphrase_results):
    """Merge paraphrase results into the dataset dataframe by vignette_id.

    Args:
        df: DataFrame with vignette_id column
        paraphrase_results: dict mapping str(vignette_id) -> {paraphrase, actual_pct, ...}

    Returns:
        DataFrame with added paraphrase_text column
    """
    df = df.copy()
    df['paraphrase_text'] = df['vignette_id'].apply(
        lambda vid: paraphrase_results.get(str(vid), {}).get('paraphrase', None)
    )
    return df


def merge_paraphrases_from_file(df, path):
    """Load paraphrase results from JSON and merge into dataframe."""
    with open(path) as f:
        paraphrase_results = json.load(f)
    return merge_paraphrases(df, paraphrase_results)
...
async def generate_one_paraphrase(vignette_id, white_text, black_text, tokenizer, openai_client,
                                  max_retries=50, tolerance=0.5):
    """Generate a calibrated paraphrase for one vignette targeting its race-swap distance.
    ...
    Args:
        vignette_id: Identifier for this vignette
        white_text: White-name version of the vignette
        black_text: Black-name version of the vignette
    ...
    Returns:
        dict with vignette_id, paraphrase, actual_pct, retries_used, deviation, all_attempts
    """
    # Target percentage from race swap distance
    target_pct = token_edit_distance_percent(white_text, black_text, tokenizer)

    # Calculate target edits
    orig_tokens = tokenizer.encode(white_text, add_special_tokens=False)
    ...
    for attempt_num in range(max_retries + 1):
        ...
        actual_pct = token_edit_distance_percent(white_text, paraphrase, tokenizer)
        ...
        if attempt_num < max_retries:
            messages.append({"role": "assistant", "content": paraphrase})
            retry_prompt = format_retry_prompt(actual_pct, target_pct, tolerance)
            messages.append({"role": "user", "content": retry_prompt})

    if not attempts:
        print(f"  WARNING: vignette_id={vignette_id} — all {max_retries + 1} attempts were refusals, using original text")
        return {
            'vignette_id': vignette_id,
            'paraphrase': white_text,
            'actual_pct': 0.0,
    ...
    if should_switch_to_synonym_strategy(attempts, target_pct, tolerance):
        ...
        synonym_prompt = format_synonym_prompt(white_text, num_words)
...
async def generate_all_paraphrases(dataset_df, output_path, tokenizer, openai_client,
                                   max_concurrent=50, checkpoint_freq=10):
    """Generate calibrated paraphrases for all vignettes with async parallelism.

    Args:
        dataset_df: DataFrame with vignette_id, white_text, black_text columns
        ...
        checkpoint_freq: Save checkpoint every N completions (default 10)

    Returns:
        dict mapping str(vignette_id) -> result dict
    """
    ...
    pending = dataset_df[~dataset_df['vignette_id'].astype(str).isin(done_ids)]
    print(f"Generating paraphrases for {len(pending)} vignettes ({len(done_ids)} already done)")
        ...
        async with semaphore:
            try:
                result = await generate_one_paraphrase(
                    vignette_id=vignette_id,
                    white_text=row['white_text'],
                    black_text=row['black_text'],
                    tokenizer=tokenizer,
                ...
                async with lock:
                    results[str(vignette_id)] = save_result
                    completed_count[0] += 1
                ...
                print(f"  ERROR vignette_id={vignette_id}: {e}")
